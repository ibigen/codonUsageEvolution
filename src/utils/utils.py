...
class Utils(object):
	...
	def get_temp_file(self, file_name, sz_type):
		...
		while 1:
			return_file = os.path.join(main_path, "cai_" + file_name + "_" + str(random.randrange(10000000, 99999999, 10)) + "_file" + sz_type)
	...
	def get_temp_file_from_dir(self, dir_out, file_name, sz_type):
		...
		while 1:
			return_file = os.path.join(dir_out, "cai_" + file_name + "_" + str(random.randrange(10000000, 99999999, 10)) + "_file" + sz_type)
			if (os.path.exists(return_file)): continue
			try:
				os.close(os.open(return_file, os.O_CREAT | os.O_EXCL))
				return return_file
	...
	def get_aminos(self, b_add_all = False):
		...
		for codon in ct.unambiguous_dna_by_id[self.genetic_code].forward_table:
			...
			if amino in dt_amino: dt_amino[amino].append(codon)
			else: dt_amino[amino] = [codon]
